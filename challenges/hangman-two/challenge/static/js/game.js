...
function resetOutcomeDivs() {
    const outcomeDivs = document.querySelectorAll('.outcome');
    if (outcomeDivs) {
        outcomeDivs.forEach(div => {
            div.textContent = '';
        });
    }
}
    ...
    $.get(`/guess?letter=${encodeURIComponent(alphabetId)}`, function(data, textStatus, jqXHR) {
        ...
        if (data.reload) {
            ...
            if (data.present === 'true') {
                ...
                const outcomeDiv = document.querySelector(".outcome.alert.alert-info");
                if (outcomeDiv) {
