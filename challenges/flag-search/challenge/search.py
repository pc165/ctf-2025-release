...
key_bytes = hashlib.md5(b'symmetric').digest()
bekey = int.from_bytes(key_bytes)

key = int('{:0128b}'.format(bekey)[::-1], 2)

print(f"key in hex: {key :x}")
#              0123456789abcdef
flag_bytes = b'CTF{bctrckngftw}'
...
def check_key(key, checks):

    c = 0
    for num, check in checks.items():
        c += 1
        if not check(key):
            print(f"Key failed check {num}!")
            return

    print(f"Key passed all {c} checks")

    aes = AES.new(key_bytes, AES.MODE_ECB)
    enc_flag = aes.encrypt(flag_bytes)
...
def search(lim, checks):
    ...
    def search_rec(d, v, pwr):
        ...
        search_rec(d + 1, v, pwr << 1)       # zero
        search_rec(d + 1, v + pwr, pwr << 1) # one
